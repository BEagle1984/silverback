...
namespace Silverback.Messaging.Configuration
{
    ...
    public static class BrokerOptionsBuilderExtensions
    {
        ...
        /// </summary>
        public static BrokerOptionsBuilder AddDbInboundConnector<TDbContext>(this BrokerOptionsBuilder builder) where TDbContext : DbContext
        {
            builder.AddInboundConnector<LoggedInboundConnector>();
            builder.Services.AddScoped<IInboundLog>(s => new DbContextInboundLog(s.GetRequiredService<TDbContext>()));
        ...
        /// </summary>
        public static BrokerOptionsBuilder AddDbOutboundConnector<TDbContext>(this BrokerOptionsBuilder builder) where TDbContext : DbContext
        {
            builder.AddOutboundConnector<DeferredOutboundConnector>();
            builder.Services.AddScoped<IOutboundQueueProducer>(s => new DbContextOutboundQueueProducer(s.GetRequiredService<TDbContext>()));
        ...
        public static BrokerOptionsBuilder AddDbOutboundWorker<TDbContext>(this BrokerOptionsBuilder builder, bool enforceMessageOrder = true, int readPackageSize = 100, bool removeProduced = false) where TDbContext : DbContext
        {
            ...
            builder.Services.AddScoped<IOutboundQueueConsumer>(s => new DbContextOutboundQueueConsumer(s.GetRequiredService<TDbContext>(), removeProduced));

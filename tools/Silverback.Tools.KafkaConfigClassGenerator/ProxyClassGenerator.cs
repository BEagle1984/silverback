...
internal sealed class ProxyClassGenerator
{
    ...
    private void GenerateHeading()
    {
        ...
        _builder.AppendLine("    /// </summary>");
        _builder.AppendLine("    [SuppressMessage(\"\", \"SA1649\", Justification = \"Autogenerated all at once\")]");
        _builder.AppendLine("    [SuppressMessage(\"\", \"SA1402\", Justification = \"Autogenerated all at once\")]");
        _builder.AppendLine("    [SuppressMessage(\"\", \"CA1200\", Justification = \"Summary copied from wrapped class\")]");
        _builder.AppendLine("    [SuppressMessage(\"\", \"SA1623\", Justification = \"Summary copied from wrapped class\")]");
        _builder.AppendLine("    [SuppressMessage(\"\", \"SA1629\", Justification = \"Summary copied from wrapped class\")]");
        _builder.AppendLine("    [SuppressMessage(\"\", \"CA1044\", Justification = \"Accessors generated according to wrapped class\")]");

        if (_baseClassName == null)
        {
            _builder.AppendLine($"    public abstract class {_generatedClassName} : IValidatableEndpointSettings");
            _builder.AppendLine("    {");
            _builder.AppendLine("        internal static readonly ConfigurationDictionaryEqualityComparer<string, string> ConfluentConfigEqualityComparer = new();");
            _builder.AppendLine();
            _builder.AppendLine("        /// <summary>");
    ...
    private void MapProperties()
    {
        ...
        {
            string propertyType = GetPropertyTypeString(property.PropertyType);
    ...
    private void WriteSummary(PropertyInfo propertyInfo)
    {
        ...
        XmlNode? node = _xmlDoc?.SelectSingleNode("//member[starts-with(@name, '" + path + "')]");
    ...
    private void WriteCustomRemarks(string propertyName)
    {
        if (propertyName == "DeliveryReportFields")
        {
            ...
            _builder.AppendLine("            ///     Silverback overrides this value by default setting it to &quot;key,status&quot; as an optimization,");

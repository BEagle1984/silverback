...
`Serializer` | The `IMessageSerializer` to be used to deserialize the messages. The default is a `JsonMessageSerializer` using UTF-8 encoding. See <xref:serialization> for details.
`Encryption` | Enable end-to-end message encryption. See <xref:encryption> for details.
`Chunk` | Enable chunking to efficiently deal with large messages. See <xref:chunking> for details.

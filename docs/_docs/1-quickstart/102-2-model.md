...
The `IDomainEvent` extends `IEvent` and the types implementing this interface are usually published only from within the domain entities (actually adding them to the internal collection and letting them be published during the save changes transaction). See also [DDD and Domain Events]({{ site.baseurl }}/docs/quickstart/ddd).

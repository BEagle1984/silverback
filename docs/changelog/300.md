...
* Simplify configuration and reduce boilerplate
    * Simplify subscribers registration and get rid of the `ISubscriber` interface (see <xref:bus>)
...
* Refactored <xref:Silverback.Messaging.Publishing.IPublisher>
    * Removed the overloads to publish a batch of messages (see <xref:bus>)
    * Cannot subscribe to collection of messages anymore (see <xref:bus>), unless they are consumed from a message broker (see <xref:consuming>)
...
* Removed `BusConfigurator` (moved all the configuration into the `ISilverbackBuilder` extension methods)
    ...
    * Replaced `BusConfigurator.Subscribe` methods with `ISilverbackBuilder.AddDelegateSubscriber` (see <xref:bus>)
    * Replaced `BusConfigurator.HandleMessagesOfType` methods with `ISilverbackBuilder.HandleMessageOfType` (see <xref:bus>)
...
* Some changes to the behaviors:
    ...
    * Changed signature of the `HandleAsync` method (see <xref:bus>, <xref:producer-behaviors> and <xref:consumer-behaviors>)

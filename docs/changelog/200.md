...
* Add support for multiple message brokers (Kafka and RabbitMQ) in the same application (see <xref:broker>)
* Add end-to-end message encryption (see <xref:encryption> and <xref:decryption>)
* Add dynamic custom routing of outbound messages (see <xref:producing>)
* Improve support for message headers (see <xref:producing-headers> and <xref:consuming-headers>)
* Add support for binary files (see <xref:serialization> and <xref:deserialization>)
...
* `x-first-chunk-offset` header added by default (see <xref:default-headers>)
...
* Add `LogWithLevel` method to <xref:Silverback.Messaging.Consuming.ErrorHandling.SkipMessageErrorPolicy> to specify the desired level for the "Message skipped" log entry (the default is now increased to `Error`)
...
* The `IBroker` interface and `Broker` abstract base class have been modified to explicitly declare which endpoint type is being handled by the broker implementation
...
* `WithConnectionTo<>`, `WithConnectionToKafka` and `WithConnectionToRabbitMQ` are deprecated (they will still be supported in this version), please use the new `WithConnectionToMessageBroker` and `AddKafka`/`AddRabbit` methods (see <xref:broker>)

## [2.0.0](https://github.com/BEagle1984/silverback/releases/tag/v2.0.0)

### What's New

* Create [Silverback.Integration.RabbitMQ](https://www.nuget.org/packages/Silverback.Integration.RabbitMQ) package to connect Silverback with RabbitMQ
...
* Add hook to manually set the Kafka partition start offset when a partition is assigned to the consumer (see <xref:broker-callbacks>) [[#57](https://github.com/BEagle1984/silverback/issues/57)]
* Support for multiple consumer groups running in the same process [[#59](https://github.com/BEagle1984/silverback/issues/59)]
...
* The `IBroker`, <xref:Silverback.Messaging.Broker.IProducer> and <xref:Silverback.Messaging.Broker.IConsumer> interfaces have been slightly modified (it shouldn't affect you unless you built your own `IBroker` implementation)

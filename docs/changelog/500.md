...
## [5.5.3](https://github.com/BEagle1984/silverback/releases/tag/v5.5.3)

### Fixes

* Prevent `ObjectDisposedException` in <xref:Silverback.Messaging.Broker.IBrokerClientsConnector> when application shutdown calls overlap

## [5.5.2](https://github.com/BEagle1984/silverback/releases/tag/v5.5.2)

### Fixes

* Make <xref:Silverback.Messaging.Broker.IBrokerClientsConnector> idempotent and safe to call concurrently

## [5.5.1](https://github.com/BEagle1984/silverback/releases/tag/v5.5.1)

### Fixes

* Restore the <xref:Silverback.Messaging.Serialization.NewtonsoftJsonMessageDeserializer`1> stream position after deserialization for backward compatibility

## [5.5.0](https://github.com/BEagle1984/silverback/releases/tag/v5.5.0)

### What's New

* Reduce <xref:Silverback.Messaging.Serialization.NewtonsoftJsonMessageSerializer> and <xref:Silverback.Messaging.Serialization.NewtonsoftJsonMessageDeserializer`1> allocations

### Fixes

* Prevent lazy message stream pushes from hanging when the provider is completed or aborted before the stream is created

## [5.4.2](https://github.com/BEagle1984/silverback/releases/tag/v5.4.2)

### Fixes

* Correctly route tombstones and regular messages to their respective subscribers regardless of which message is consumed first

## [5.4.1](https://github.com/BEagle1984/silverback/releases/tag/v5.4.1)

### Fixes

* Prevent a race condition that could cause the <xref:Silverback.Messaging.Sequences.Batch.BatchSequence> constructor to throw an `ObjectDisposedException`

## [5.4.0](https://github.com/BEagle1984/silverback/releases/tag/v5.4.0)

### What's New

* Upgrade to [Confluent.Kafka 2.15.0](https://github.com/confluentinc/confluent-kafka-dotnet/releases/tag/v2.15.0)
* Upgrade to [MQTTnet 5.2.0](https://github.com/dotnet/MQTTnet/releases/tag/v5.2.0)

### Fixes

* Optimize the shutdown process by stopping consumers in parallel
* Correctly handle subscriber tasks completion
* Resolve rebalance race condition in the mocked Kafka consumer

## [5.3.3](https://github.com/BEagle1984/silverback/releases/tag/v5.3.3)
...
## [5.3.2](https://github.com/BEagle1984/silverback/releases/tag/v5.3.2)

### Fixes

* Prevent deadlock in the disconnect process

## [5.3.1](https://github.com/BEagle1984/silverback/releases/tag/v5.3.1)
...
## [5.3.0](https://github.com/BEagle1984/silverback/releases/tag/v5.3.0)

### What's New

* Upgrade to [Confluent.Kafka 2.14.0](https://github.com/confluentinc/confluent-kafka-dotnet/releases/tag/v2.14.0)

### Fixes

* Remove the `chunk-message-id` header, which was added by mistake in v5 and introduced an unintended break

## [5.2.2](https://github.com/BEagle1984/silverback/releases/tag/v5.2.2)

### Fixes

* Handle some potential race conditions in the batch completion and timeout handling

## [5.2.1](https://github.com/BEagle1984/silverback/releases/tag/v5.2.1)

### Fixes

* Fix a potential deadlock in the batch sequence completion

## [5.2.0](https://github.com/BEagle1984/silverback/releases/tag/v5.2.0)

### What's New

* Add `WithKafkaTopic` and `WithMqttTopic` extension methods for the <xref:Silverback.Messaging.Messages.InboundEnvelopeBuilder`1> (see <xref:testing>)
* Add <xref:Silverback.Messaging.Publishing.IIntegrationPublisher> as an alternative to the `WrapAndPublish` extension methods on <xref:Silverback.Messaging.Publishing.IPublisher> (see <xref:producing>)
...
* Add <xref:Silverback.Messaging.Publishing.IApplicationPublisher> as an alternative to the extension methods on <xref:Silverback.Messaging.Publishing.IPublisher> (see <xref:bus>)
* Upgrade to [Confluent.Kafka 2.13.2](https://github.com/confluentinc/confluent-kafka-dotnet/releases/tag/v2.13.2)
...
* Add `CancellationToken` support to the message bus (see <xref:bus>)
...
* Some changes to the default settings:
  ...
  * The JSON deserializers don't use the `x-message-type` header anymore by default, meaning that you have to explicitly activate the feature to deserilize polymorphic messages (see <xref:deserialization>)
  * Changed default <xref:Silverback.Messaging.Serialization.NewtonsoftJsonMessageSerializer> and <xref:Silverback.Messaging.Serialization.NewtonsoftJsonMessageDeserializer`1> typename handling to `TypeNameHandling.None`, meaning that you need to explicitly set the type name handling strategy if you want to support polymorphic nested properties
  * Reverted default and null value handling in <xref:Silverback.Messaging.Serialization.NewtonsoftJsonMessageSerializer> to Newtonsoft.Json's default
  * By default, the `MqttProducer` will not throw an exception anymore if no subscriber is consuming the produced message (can be configured to log a warning, or throw an exception)
...
* Replace usage of `IEventPublisher`, `ICommandPublisher`, and `IQueryPublisher` with the related extension methods on the basic <xref:Silverback.Messaging.Publishing.IPublisher> or the consolidated <xref:Silverback.Messaging.Publishing.IApplicationPublisher> (introduced in [v5.1.0](#510)) if you prefer a more testable approach (see also <xref:bus>)
* If using the outbox, adjust the DbContext / database schema and review transaction handling: you might need to explicitly begin a transaction and enlist it in the publisher to keep application changes and outbox messages atomic (see also <xref:outbox>)

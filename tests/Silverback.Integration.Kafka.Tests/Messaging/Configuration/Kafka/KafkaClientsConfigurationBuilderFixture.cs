...
public class KafkaClientsConfigurationBuilderFixture
{
    ...
    public async Task AddProducer_ShouldMergeProducerConfiguration_WhenIdIsTheSame()
    {
        ...
        producers[0].EndpointConfiguration.MessageType.Should().Be<TestEventOne>();
        producers[0].EndpointConfiguration.As<KafkaProducerEndpointConfiguration>().Endpoint.RawName.Should().Be("topic1");
        producers[0].As<KafkaProducer>().Configuration.BatchSize.Should().Be(42);
        producers[0].As<KafkaProducer>().Configuration.LingerMs.Should().Be(42);
        producers[1].EndpointConfiguration.MessageType.Should().Be<TestEventTwo>();
        producers[1].EndpointConfiguration.As<KafkaProducerEndpointConfiguration>().Endpoint.RawName.Should().Be("topic2");
        producers[1].As<KafkaProducer>().Configuration.BatchSize.Should().Be(42);
        producers[1].As<KafkaProducer>().Configuration.LingerMs.Should().Be(42);
    ...
    public async Task AddConsumer_ShouldMergeConsumerConfiguration_WhenIdIsTheSame()
    {
        ...
        ConsumerCollection consumers = serviceProvider.GetRequiredService<ConsumerCollection>();
        consumers.Should().HaveCount(1);
        consumers[0].As<KafkaConsumer>().Configuration.Endpoints.Should().HaveCount(2);
        consumers[0].As<KafkaConsumer>().Configuration.Endpoints.First().TopicPartitions.Should().HaveCount(1);
        consumers[0].As<KafkaConsumer>().Configuration.Endpoints.First().RawName.Should().Be("topic1");
        consumers[0].As<KafkaConsumer>().Configuration.Endpoints.Skip(1).First().TopicPartitions.Should().HaveCount(1);
        consumers[0].As<KafkaConsumer>().Configuration.Endpoints.Skip(1).First().RawName.Should().Be("topic2");
        consumers[0].As<KafkaConsumer>().Configuration.GroupId.Should().Be("group1");
        consumers[0].As<KafkaConsumer>().Configuration.EnablePartitionEof.Should().BeTrue();
        consumers[0].As<KafkaConsumer>().Configuration.FetchMinBytes.Should().Be(42);
    }

...
namespace Silverback.Tests.Performance.Broker
{
    ...
    public static class ProduceStrategiesComparisonRunner
    {
        ...
        {
            var statsCollection = new List<ProduceStrategiesImplementation.Stats>();

            if (MustExecute(1, startStrategyIndex, endStrategyIndex))
            {
                var implementation = new ProduceStrategiesImplementation();
            ...
            if (MustExecute(3, startStrategyIndex, endStrategyIndex))
            {
                var implementation = new ProduceStrategiesImplementation();
                Console.WriteLine(
                    $"Producing {iterations} messages using Publisher.PublishAsync no await...");
            ...
            if (MustExecute(4, startStrategyIndex, endStrategyIndex))
            {
                var implementation = new ProduceStrategiesImplementation();
                Console.WriteLine(
                    $"Producing {iterations} messages using Producer.ProduceAsync no await...");
            ...
            if (MustExecute(5, startStrategyIndex, endStrategyIndex))
            {
                var implementation = new ProduceStrategiesImplementation();
                Console.WriteLine(
                    $"Producing {iterations} messages using Producer.Produce with callbacks...");
            ...
            if (MustExecute(6, startStrategyIndex, endStrategyIndex))
            {
                var implementation = new ProduceStrategiesImplementation();
                Console.WriteLine(
                    $"Producing {iterations} messages using Producer.Produce with callbacks, wrapped in Task.Run...");

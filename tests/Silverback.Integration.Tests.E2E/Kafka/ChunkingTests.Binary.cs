...
public partial class ChunkingTests
{
    ...
    public async Task Chunking_ShouldConsumeChunkedBinaryMessageWithMessageTypeHeader()
    {
        ...
        for (int i = 1; i <= 3; i++)
        {
            ...
            await producer.RawProduceAsync(
                rawMessage.Take(3).ToArray(),
                envelope => envelope.SetChunkHeaders("1", 0, 3, typeof(BinaryMessage)));
            await producer.RawProduceAsync(
                rawMessage.Skip(3).Take(3).ToArray(),
                envelope => envelope.SetChunkHeaders("1", 1, 3, typeof(BinaryMessage)));
            await producer.RawProduceAsync(
                rawMessage.Skip(6).ToArray(),
                envelope => envelope.SetChunkHeaders("1", 2, 3, typeof(BinaryMessage)));
        }

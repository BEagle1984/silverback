...
public class KafkaTransactionsTests : KafkaTests
{
    ...
    public async Task KafkaTransactions_ShouldSendOffsetsToTransactionAndCommit()
    {
        ...
        async ValueTask HandleInput(TestEventOne eventOne, IPublisher publisher)
        {
            ...
            await publisher.PublishEventAsync(new TestEventTwo { ContentEventTwo = eventOne.ContentEventOne });
            int i = Interlocked.Increment(ref processedInputMessages);
        ...
        // The first 3 messages don't commit the transaction
        await AsyncTestingUtil.WaitAsync(() => processedInputMessages >= 3);
        Helper.GetConsumerGroup("input").GetCommittedOffsetsCount("input").ShouldBe(0);
        ...
        Helper.GetConsumerGroup("input").GetCommittedOffsetsCount("input").ShouldBe(6);
    }

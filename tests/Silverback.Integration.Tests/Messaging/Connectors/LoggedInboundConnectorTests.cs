    ...
    public class LoggedInboundConnectorTests
    {
        ...
        public async Task Bind_PushMessages_MessagesReceived()
        {
            ...
            await consumer.TestHandleMessage(
                new TestEventOne(),
                new[] { new MessageHeader("x-message-id", Guid.NewGuid()) });
            await consumer.TestHandleMessage(
                new TestEventOne(),
                new[] { new MessageHeader("x-message-id", Guid.NewGuid()) });
        ...
        public async Task Bind_PushMessages_EachIsConsumedOnce()
        {
            ...
            var consumer = (TestConsumer)_broker.Consumers[0];
            await consumer.TestHandleMessage(e1, new[] { new MessageHeader("x-message-id", id1) });
            await consumer.TestHandleMessage(e2, new[] { new MessageHeader("x-message-id", id2) });
            await consumer.TestHandleMessage(e1, new[] { new MessageHeader("x-message-id", id1) });
            await consumer.TestHandleMessage(e2, new[] { new MessageHeader("x-message-id", id2) });
            await consumer.TestHandleMessage(e1, new[] { new MessageHeader("x-message-id", id2) });
        ...
        public async Task Bind_PushMessagesInBatch_OnlyCommittedBatchWrittenToLog()
        {
            ...
            try
            {
                await consumer.TestHandleMessage(e1, new[] { new MessageHeader("x-message-id", Guid.NewGuid()) });
            }
            catch
            {
                // ignored
            ...
            try
            {
                await consumer.TestHandleMessage(e2, new[] { new MessageHeader("x-message-id", Guid.NewGuid()) });
            }
            catch
            {
                // ignored
            ...
            try
            {
                await consumer.TestHandleMessage(e3, new[] { new MessageHeader("x-message-id", Guid.NewGuid()) });
            }
            catch
            {
                // ignored
            ...
            try
            {
                await consumer.TestHandleMessage(e4, new[] { new MessageHeader("x-message-id", Guid.NewGuid()) });
            }
            catch
            {
                // ignored

    ...
    public class SymmetricDecryptStreamTests
    {
        ...
        public async Task ReadAsync_UsingRijndaelWithCustomSettings_MessageIsSuccessfullyDecrypted()
        {
            var cypherMessage = new MemoryStream(
                new byte[]
                {
                    0xc5, 0x70, 0xbf, 0x56, 0x82, 0xc2, 0x5f, 0x71, 0x53, 0xa2, 0xa3, 0x12, 0xd0, 0x8b, 0x0d, 0x08,
                    0xa1,
        ...
        public async Task ReadAsync_WithoutSpecifyingIV_IVIsExtractedAndMessageIsSuccessfullyDecrypted()
        {
            var cypherMessage = new MemoryStream(
                new byte[]
                {
                    0x6d, 0xda, 0xdb, 0x0c, 0xf2, 0xd9, 0xb3, 0x60, 0x77, 0xcf, 0x69, 0x09, 0x52, 0x24, 0x79, 0x0a,
                    0x82,
        ...
        [Fact]
        public async Task ReadAsync_EmptyStream_EmptyStreamIsReturned()
        {
            var cryptoStream = new SymmetricDecryptStream(
                new MemoryStream(),
                new SymmetricEncryptionSettings
        ...
        [Fact]
        public void Read_EmptyStream_EmptyStreamIsReturned()
        {
            var cryptoStream = new SymmetricDecryptStream(
                new MemoryStream(),
                new SymmetricEncryptionSettings

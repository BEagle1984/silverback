...
namespace Silverback.Messaging.Proxies
{
    public abstract  class ConfluentClientConfigProxy
    {
        ...
        ///<summary> Maximum Kafka protocol request message size. Due to differing framing overhead between protocol versions the producer is unable to reliably enforce a strict max message limit at produce time and may exceed the maximum size by one message in protocol ProduceRequests, the broker will enforce the the topic's `max.message.bytes` limit (see Apache Kafka documentation). default: 1000000 importance: medium </summary>
        ...
        ///<summary> A rack identifier for this client. This can be any string value which indicates where this client is physically located. It corresponds with the broker config `broker.rack`. default: '' importance: low </summary>
    ...
    public class ConfluentConsumerConfigProxy : ConfluentClientConfigProxy
    {
        ...
        ///<summary> Controls how to read messages written transactionally: `read_committed` - only return transactional messages which have been committed. `read_uncommitted` - return all messages, even transactional messages which have been aborted. default: read_committed importance: high </summary>

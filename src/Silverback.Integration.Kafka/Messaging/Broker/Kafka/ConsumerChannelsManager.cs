    ...
    internal sealed class ConsumerChannelsManager : IDisposable
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (consumeResult.IsPartitionEOF)
            {
                ...
                if (_consumer.Endpoint.Configuration.EnablePartitionEof.HasValue &&
                    _consumer.Endpoint.Configuration.EnablePartitionEof.Value)
                {
                    _callbacksInvoker.Invoke<IKafkaEndOfTopicPartitionReachedCallback>(

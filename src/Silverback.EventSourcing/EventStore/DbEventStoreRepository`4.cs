    ...
    where TEventEntity : class, IEventEntity, new()
{
    ...
    {
        TEventStoreEntity? eventStoreEntity = EventStores.FirstOrDefault(predicate);
    ...
    {
        TEventStoreEntity? eventStoreEntity = await EventStores.FirstOrDefaultAsync(predicate).ConfigureAwait(false);

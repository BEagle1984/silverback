...
{
    /// <summary>
    ///     Can be placed on a subscribed method to filter the messages to be processed according to the client
    ///     id that consumed them. This is used when having multiple clients for the same topic running
    ///     in the same process.
    /// </summary>
    public sealed class MqttClientIdFilterAttribute : MessageFilterAttribute
    {
        ...
        public override bool MustProcess(object message) =>
            ...
            inboundEnvelope.Endpoint is MqttConsumerEndpoint endpoint &&
            ClientId.Any(groupId => groupId == endpoint.Configuration.ClientId);
    }
}

...
internal sealed class MqttClientsCache : IMqttClientsCache
{
    ...
    {
        MqttClientWrapper client = GetClient(consumer.Configuration.Client, true);
    ...
    private MqttClientWrapper GetClient(MqttClientConfiguration clientConfiguration, bool isForConsumer)
    {
        ...
        lock (_clients)
        {
            if (_clients.TryGetValue(clientConfiguration.ClientId, out MqttClientWrapper? client))
            {
                if (!client.ClientConfiguration.Equals(clientConfiguration))
                {
                    throw new InvalidOperationException("A client with the same id is already connected but with a different configuration.");
                }

                if (isForConsumer && client.Consumer != null)
                {
                    throw new InvalidOperationException("Cannot use the same client id for multiple consumers.");
                }
            }
            else
            {

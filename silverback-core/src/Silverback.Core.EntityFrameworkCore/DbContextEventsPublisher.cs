...
namespace Silverback.EntityFrameworkCore
{
    ...
    public static class DbContextEventsPublisher
    {
        ...
        /// </summary>
        public static int ExecuteSaveTransaction(DbContext dbContext, Func<int> saveChanges, IEventPublisher<IEvent> eventPublisher)
            => ExecuteSaveTransaction(dbContext, () => Task.FromResult(saveChanges()), eventPublisher, false).Result;

        /// <summary>
        /// Publishes the domain events generated by the tracked entities and then executes the provided save changes procedure.
        /// </summary>
        public static Task<int> ExecuteSaveTransactionAsync(DbContext dbContext, Func<Task<int>> saveChangesAsync, IEventPublisher<IEvent> eventPublisher)
            => ExecuteSaveTransaction(dbContext, saveChangesAsync, eventPublisher, true);

        private static async Task<int> ExecuteSaveTransaction(DbContext dbContext, Func<Task<int>> saveChanges, IEventPublisher<IEvent> eventPublisher,  bool async)
        {
        ...
        private static async Task PublishDomainEvents(DbContext dbContext, IEventPublisher<IEvent> eventPublisher, bool async)
        {
        ...
        private static Task PublishEvent<TEvent>(IEventPublisher<IEvent> eventPublisher, bool async)
